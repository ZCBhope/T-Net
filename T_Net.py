...
from utils import batch_episym,batch_symeig,weighted_8points
...
class T_Net(nn.Module):
    def __init__(self, config):
        ...
        self.subnetwork_init = sub_T(config.net_channels, 4, depth_each_stage
                                     , config.clusters,config.model)
        self.subnetwork = [sub_T(config.net_channels, 6, depth_each_stage , config.clusters ,config.model) for _ in range(self.iter_num)]
